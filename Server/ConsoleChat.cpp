...
void ConsoleChat::checkConnection() {
	_сhatStarted = ns.checkConnection();
}
...
void ConsoleChat::startMenu() // Стартовое меню, отображается при запуске
{
	...
	ns >> str;
	if (ns.checkConnection()) {
		this->_сhatStarted = false;
		return;
	}
...
shared_ptr<User> ConsoleChat::getUserByLogin(const string login) const
{
	...
	sql::ResultSet* res = 0;
	res = stmt->executeQuery("SELECT id,create_time,name,login,password,status FROM table_users");
	while (res->next()) {
		...
		if (clogin == login) {
			...
			int cstatus = res->getInt("status");
			result = shared_ptr<User>(new User(cid, clogin, cpwd, cname, cstatus));
			break;
		}
	}
	delete res;
...
void ConsoleChat::logIn() // Вход в чат
{
	...
	if (_onlineUser) {
		if (_onlineUser->getStatus() != 0) {			
			switch (_onlineUser->getStatus())
			{
			case 1:
				return;
			case 2:
				ns << "User banned. Access denied";
				break;
			default:
				ns << "User status = " << std::to_string(_onlineUser->getStatus());
				break;
			}
			ns << "\n";
			_onlineUser = nullptr;
...
void ConsoleChat::signUp() // Регистрация
{
	...
	User user = User(-1, login, hashedPassword, name, 0); // Создаем нового пользователя

	shared_ptr<User> result = nullptr;
	sql::Statement* stmt = con->createStatement();
	stmt->execute("insert into table_users (name, login, password, status) values('" +
		name + "','" +
		login + "','" +
		hashedPassword + "','" +
		"0" + "')");
...
{
	if (_onlineUser) {
		// Создаем SQL-запрос для получения статуса пользователя
		sql::Statement* stmt = con->createStatement();
		sql::ResultSet* res = stmt->executeQuery("SELECT status FROM table_users WHERE login = '" + _onlineUser->getLogin() + "'");

		if (res->next()) {
			int userStatus = res->getInt("status");

			// Проверяем статус пользователя
			if (userStatus == 2) {
				ns << "User banned. Access denied ";
				ns << "You cannot view messages with this status." << "\n";

				// Очищаем ресурсы и выходим из функции
				delete res;
				delete stmt;
				return;
			}
	...
	sql::Statement* stmt = con->createStatement();
	sql::ResultSet* res = 0;
	res = stmt->executeQuery("SELECT id,create_time,name,login,password FROM table_users where status<>1");
	while (res->next()) {
		string clogin = res->getString("login");
		string cpwd = res->getString("password");
		string cname = res->getString("name");
...
void ConsoleChat::sendMessage() // Написать сообщение
{
	if (_onlineUser) {
		...
		if (res->next()) {
			...
			if (userStatus == 1 || userStatus == 2) {
				...
				ns << "You cannot send messages with this status." << "\n";
	...
	ns << ">> ";
	ns >> text;
	shared_ptr<User> rcuser = 0;
	if (!(recipient == "all")) {
		rcuser = getUserByLogin(recipient);
	}
	if (!(recipient == "all" || rcuser)) {  // Если получатель не равен all или не найден логин пользователя
	...
	if (rcuser) {
		if (rcuser->getStatus() != 0) {
			ns << "error, user is banned or blocked  -- " << recipient << "\n";
			return;
		}
	}
